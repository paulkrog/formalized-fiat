...
(* Substitution of types in ADTs preserves typing.

   Note that substituting a type into an ADT can instantiate
   type variables, so we also need to perform the substitution
   to the result type.
 *)
Theorem subst_type_adt_ix
 :  forall ix ke te ad t1 t2 k2
 ,  get ix ke = Some k2
 -> TYPEADT ke  te ad t1
 -> KIND (delete ix ke)  t2 k2
 -> TYPEADT (delete ix ke) (substTE ix t2 te)
         (substTADT ix t2 ad) (substTT ix t2 t1).
Proof.
 intros. gen ix ke te t1 t2 k2.
 induction ad; intros; simpl; inverts H0; eauto.
 Case "ADT".
 apply TYADT. Check TYADT.
Abort.
