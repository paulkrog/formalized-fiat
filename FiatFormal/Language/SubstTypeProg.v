...
(* Substitution of types in programs preserves typing.

   Note that substituting a type into a program can instantiate
   type variables, so we also need to perform the substitution
   to the result type.
 *)
Theorem subst_type_prog_ix
 :  forall ix ke te p t1 t2 k2,
    get ix ke = Some k2
    -> TYPEPROG ke te p t1
    -> KIND (delete ix ke)  t2 k2
    -> TYPEPROG (delete ix ke) (substTE ix t2 te)
               (substTP ix t2 p) (substTT ix t2 t1).
Proof.
 intros. gen ix ke te t1 t2 k2.
 induction p; intros; simpl; inverts H0; eauto.
 Case "PLet".
 eapply TYLet.
 Focus 2.
